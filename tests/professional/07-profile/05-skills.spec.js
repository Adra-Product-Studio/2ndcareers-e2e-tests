...
test.describe("Professional - Profile - Skills", () => {
  const session = useSharedPage(test, { videoName: "07-profile-05-skills" });
  ...
  test("adds a temporary skill, then deletes it - the account's real skills are untouched", async () => {
    const profile = new ProfessionalProfilePage(session.page);
    await profile.waitForLoad();
    await profile.addThenDeleteSkill();
  });
});
