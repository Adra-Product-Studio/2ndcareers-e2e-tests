...
test.describe("Professional - Jobs - Recommended", () => {
  ...
  test("below the 60% profile-completion threshold, shows the complete-your-profile prompt instead of fetching jobs", async () => {
    ...
    // 01-all-jobs.spec.js left the session on the All Jobs tab - arrive here via a real click on
    // the Recommended tab rather than a fresh page.goto().
    await recommended.waitForLoadBelowThreshold(() => recommended.recommendedTab.click());
  });
});
